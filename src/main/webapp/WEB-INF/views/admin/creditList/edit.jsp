...
<div class="panel panel-default" style="float: left; width: 90%; border-radius: 0px;">
    ...
    <div class="panel-body">
        ...
        <form class="form-inline" style="margin-left: 10px" action="${contextPath}/admin/creditList/edit.action" method="post">
            <input type="hidden" name="creditListId" id="creditListId" value="${credit.creditListId}">
            <div class="form-group">
                <label class="control-label" for="listState">请选择状态</label>
                <select class="form-control" id="listState" name="listState">
                    <c:if test="${credit != null}">
                        <c:if test="${credit.listState == 1}">
                            <option value="1" selected>未审核</option>
                            <option value="2">审核通过</option>
                            <option value="3">审核未通过</option>
                        </c:if>
                        <c:if test="${credit.listState == 2}">
                            <option value="1">未审核</option>
                            <option value="2" selected>审核通过</option>
                            <option value="3">审核未通过</option>
                        </c:if>
                        <c:if test="${credit.listState == 3}">
                            <option value="1">未审核</option>
                            <option value="2">审核通过</option>
            ...
            <button type="button" id="backMark" class="btn btn-default" style="float: right;margin-right: 100px">返回</button>
        </form>
    </div>
</div>
